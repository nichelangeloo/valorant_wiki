...
const Pagination = ({results, query, currentPage, setCurrentPage, filterParam}) => {
    ...
    .filter(result => {   // Filtering by category
      if(result.role.displayName === filterParam) {
        return result
    ...
    .map((result, id) => {    // Passing our filtration and slicing into component
      const { uuid } = result
        return(
            <AgentCards uuid={uuid} page='/' key={id} result={result}/>
        )
    })
